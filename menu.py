...
from user_database import user_list, add_user
...
from classes import Question, Lecturer, Clause

user_menu = {'1': "LOGIN", '2': "EXIT"}
student_menu = {'1': "EXIT", '2': "CHOOSE A QUESTION", '3': "SHOW A QUESTIONS", '4': "FILTER A QUESTION",
                '5': "SEARCH A QUESTION"}
lecturer_menu = dict()
lecturer_menu.update({'6': "CREATE A TEST", '7': "ADD CLAUSES TO QUESTION"})
...
def write_log(doc):
    ...
    logFile.write("bla")
    logFile.write(str(datetime.now()) + doc + "\n")
...
def login():
    ...
    while True:
        ...
        user = tuple()
        print("\t***  LOGIN ***")
        write_log(" :get accsess to system")
        if is_login_name():
            if is_access_granted(user[1]):
                write_log(" :logged in to system")
                return user
...
def creat_question():
    q = Question()
    write_log(" :user story - add question")
    ...
    print("please enter the number of the clauses in the question:")
    q.set_clauses_num(input())
    ...
    write_log(" :user story - update format")
    print("please enter the question format of the question")
    q.set_question_format(input())
    write_log(" :question's format updated")

    add_question(q)
    save_ques_db_file(ques)
    write_log(" :question added to database")


def add_teacher():
    t = Lecturer()
    write_log(" :user story - adding teacher")

    print("please enter the teacher full name:")
    t.set_name(input())
    print("please enter the teacher phone number:")
    t.set_phone(input())
    print("please enter the teacher main topic lectuer:")
    t.set_topic(input())
    print("please set the teacher password:")
    t.set_password(input())

    add_user(t)
    save_user_db_file(user_list)
    write_log(" :teacher added to database")


def add_clauses():
    print("please enter the serial number for the question you want to work on:")
    q_serial_number = input()
    clauses_number = int(ques[q_serial_number][3])
    clauses_list = []
    for _ in range(0, clauses_number):
        c = Clause()
        c.set_clause_number(_ + 1)
        print(" insert the difficulty for the " + _ + " clause:")
        c.set_clause_difficulty(input())
        print(" insert the " + str(_) + " subject:")
        c.set_clause_subject(input())
        clauses_list.append(c)

    """ need to build the function update question and after doing that, we will update the relavente question and 
    than add the clauses_list and add it to the database"""
